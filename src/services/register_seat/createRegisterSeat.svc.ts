...
class CreateRegisterSeatService{
  ...
  async execute({id_patient, id_protocol, id_seat, checkin_timestamp, checkin_professional, checkout_timestamp, checkout_professional, notes}: IRegisterSeatCreate): Promise<IRegisterSeat>{

    const createRegisterSeat = await this.registerSeatRepository.createRegisterSeat({
      id_patient,
      id_protocol,
      id_seat,
      checkin_timestamp,
